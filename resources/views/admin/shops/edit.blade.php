    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.shops.update", [$shop->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                        @foreach($users as $id => $user)
                            <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $shop->user->id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('short_name') ? 'is-invalid' : '' }}" type="text" name="short_name" id="short_name" value="{{ old('short_name', $shop->short_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('location') ? 'is-invalid' : '' }}" type="text" name="location" id="location" value="{{ old('location', $shop->location) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('industry') ? 'is-invalid' : '' }}" type="text" name="industry" id="industry" value="{{ old('industry', $shop->industry) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $shop->description) }}">
...
@section('scripts')
    <script>
        Dropzone.options.photoDropzone = {
            ...
            init: function () {
                @if(isset($shop) && $shop->photo)
                var file = {!! json_encode($shop->photo) !!}
        ...
        $('#short_name').val(slugify($('#short_name').val()));

        $('#short_name').on('change', function (){
            let short_name = $(this).val()
            $(this).val(slugify(short_name));
        })
    </script>
@endsection
